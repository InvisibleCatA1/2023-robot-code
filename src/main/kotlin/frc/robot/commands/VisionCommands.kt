...
class AlignToTarget(val driveSystem: SwerveSubsystem, val gyro: GyroSubsystem): CommandBase() {
    val targetPose = VisionUtils.getLatestResults("").targetingResults.targets_Fiducials[0]?.getTargetPose_RobotSpace()
    val robotPose = Pose3d(driveSystem.pose)
